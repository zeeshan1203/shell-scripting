...
LVER=2                                      ##ur aws launch template latest version
...
DNS_UPDATE() {
  ...
  aws route53 change-resource-record-sets --hosted-zone-id Z0455275BOCGUWER4BVO --change-batch file:///tmp/record.json | jq         ##ur route53 hosted zone id
}
